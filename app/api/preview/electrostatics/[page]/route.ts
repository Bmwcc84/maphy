...
import {
  ...
  ELECTROSTATICS_PREVIEW_PAGE_COUNT,
...
export async function GET(request: NextRequest, context: RouteContext) {
  const startedAt = readElectrostaticsPreviewToken(
  ...
  if (Date.now() >= startedAt + ELECTROSTATICS_PREVIEW_DURATION_MS) {
  ...
  if (
    ...
    pageNumber < 1 ||
    pageNumber > ELECTROSTATICS_PREVIEW_PAGE_COUNT
  ) {
  ...
  return new Response(new Uint8Array(file), {
    headers: {
      ...
      "Content-Disposition": `inline; filename="electrostatics-preview-${pageNumber}.jpg"`,
